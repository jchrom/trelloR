...
paginate = function(url, token = NULL, response = "content", on.error = "error",
  ...
  if (limit == 0) {
    ...
    repeat {
      ...
      req = get_url(
        url = modify_url(url = next_url(req), query = list(limit = 1000)),
        token = token, response = response, on.error = on.error, handle = handle,
        verbose = verbose)

      result = append(result, list(req))

      if (inherits(req, "response") && status_code(req) >= 300)
        return(req)
      if (attributes(req)$page.length < 1000)
        break
    }
  }
    ...
    for (i in page_limits(limit)) {
      ...
      req = get_url(url = modify_url(url = next_url(req), query = list(limit = i)),
        token = token, response = response, on.error = on.error, handle = handle,
        verbose = verbose)
...
get_url = function(url, token = NULL, response = "content", retry.times = 3,
                   on.error = "error", handle = NULL, verbose = FALSE) {
