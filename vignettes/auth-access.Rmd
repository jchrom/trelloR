...
A secure token is required to authenticate your access to Trello. To obtain it, login to Trello and visit `https://trello.com/app-key`. Copy both "key" and "secret" from that page and feed them to the `get_token()` function (see below). This will trigger the first-time authorization in the browser (you only have to do it once for a given project):
