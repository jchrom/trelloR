...
Back in R, feed the key and secrey to the `get_token()` function. This will
...
my_boards = get_my_boards(token = "path/to/cache/file")
my_boards = get_my_boards(token = my_token) #works even without local cache
```
...
Using environment variables or packages like [askpass](https://github.com/jeroen/askpass) are good options.

Token cache is by default stored in a file called `.httr-oauth`, but you can
change that via the `cache` argument to the `get_token()` function. Whatever the
case, you should avoid accidentaly copying or exporting this file along with
other files. To make this easier, the cache file path is automatically added to
`.gitignore`.

If you think the token might have been compromised or you simply do not want to
use it anymore, you should revoke it. Login to Trello and access your account
