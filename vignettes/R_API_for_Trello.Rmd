...
_NOTE. Accessing the data in private boards requires authorization which you can read about in the **Accessing private data** section._
...
The write access token (see the previous section) allows for adding, updating and deleting models. This is achieved by POST, PUT and DELETE http requests. Recursion for these requests is not implemented, so you can only process one model at a time.

###Adding models

The `post_model()` function allows you to create new models. At the very least, you have to specify a `model` (eg. "card"), its `id` and a `token` with permissions to write. To further specify the model to be added, you may have to use the `path` argument. The actual content of the new model (such as *name*, *decription* or *labels* in the case of a new card) has to be passed as a named list to the `body` argument.

The following example creates a new card and places it at the bottom of the list:
...
```
Consult the [Trello API reference](https://developers.trello.com/v1.0/reference) for element names. Some of the most frequently used POST requests have been implemented as wrappers for this function. Use `add_board()`, `add_card()`, `add_checklist()`, `add_comment()`, `add_label()` and  `add_list()` to create these models in Trello.
...
The `put_model()` function follows the same logic as the `post_model()` function, but can only be called on existing models. Wrappers for this function include: `update_card()`, `update_card_labels()`, `update_card_members()` and `update_item()` (for updating checklist items).
