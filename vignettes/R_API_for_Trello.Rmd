...
## Introduction
`trellor` offers a set of functions to get data from specific levels of Trello's JSON hierarchy. No other requests (such as POST or PUT) are implemented right now, so you can only GET data out of Trello. Getting data from private boards requires a secure token, which you can read about in the Authorization section.

In order to get data from a specific endpoint (like a board or card), you need to have either its ID or the ID of its parent structure. Here is an example of getting ID of Trello Development Roadmap (public board) and using it to fetch a list of cards:
...
Since we want to get data from a card, the `"card"` is the `parent`. Card updates are a type of action, so `"actions"` is the `child`. Without the `filter` specification, all the actions would be returned, not only card updates (indeed, that is what the existing `get_card_actions` does).

You can then call your custom function the regular way, supplying card ID:

```r
card_updates = fetch_card_updates(card_id)
```

## Specifying URL requests directly
...
* `filter` (but you set that before so there is no need in this case), 
...
* `bind.rows` can be quite useful as it binds all the results into single `data.frame`. If `bind.rows = FALSE`, the result will be a list with as many elements as there were pages. The latter can also come handy on the rare occasion when the responses are not perfectly formatted and make `dplyr::bind_rows` fail.
...
If you need an overview of all your boards, a good starting point is the `get_my_boards()` function, which returns a `data.frame` with board names and IDs related to a particular user.
